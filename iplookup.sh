...
usage() {
  ...
  echo "  $(basename "$0") -l <ip> [OPTIONS]"
  echo ""
  echo -e "${BOLD}Options:${RESET}"
  echo "  -l <ip>   IPv4 address to look up (required)"
  echo "  -C16      Use the /16 CIDR dataset (cidr_16.csv)"
  echo "  -C24      Use the /24 CIDR dataset (cidr_24.csv)"
  echo "            Default dataset: all_IP_networks.csv"
  echo "  -a        Show all fields including NAUTOBOT_PHONE, NAUTOBOT_ADDRESS, SITE_CODES"
  echo "  -d        Enable debug output"
  echo "  -h        Show this help message"
  echo ""
  echo -e "${BOLD}Examples:${RESET}"
  echo "  $(basename "$0") -l 10.0.0.5"
  echo "  $(basename "$0") -l 10.0.0.5 -C16"
  echo "  $(basename "$0") -l 10.0.0.5 -C24"
  echo "  $(basename "$0") -l 192.168.1.50 -d"
  echo "  $(basename "$0") -l 172.16.10.0 -C16 -a"
  echo ""
  echo -e "${BOLD}Notes:${RESET}"
  echo "  * Longest-prefix-match: the most specific network containing the IP wins."
  echo "  * CSV data files must live in the same directory as this script."
  echo "  * Works with any POSIX awk (macOS awk, gawk, mawk, nawk)."
}
...
lookup() {
  ...
  awk_out=$(awk \
    -v query_int="$query_int" \
    -v debug="$( $DEBUG && echo 1 || echo 0 )" \
    "$AWK_PROGRAM" \
    <(LC_ALL=C sed $'1s/^\xef\xbb\xbf//' "$csv_file"))
